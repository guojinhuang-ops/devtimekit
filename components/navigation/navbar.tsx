...
export default function Navbar({ pathname }: NavbarProps) {
  ...
  >>(
    () => [
      ...
      { type: 'link', href: localizedPath('/about-unix-time', locale), label: getGuidesLabel(locale) }
    ],
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/85 backdrop-blur dark:border-slate-800 dark:bg-slate-950/85">
      <div
        dir={dictionary.dir}
        className={`container-page relative flex min-h-16 items-center justify-between gap-3 py-2 ${
          isRtl ? 'md:flex-row-reverse' : ''
        }`}
      >
        <Link
          href={localizedPath('/', locale)}
          className="inline-flex items-center gap-2.5 rounded-md px-1 py-0.5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-500"
        ...
        <div ref={navRootRef} className="hidden items-start gap-3 md:flex" onMouseLeave={scheduleClose}>
